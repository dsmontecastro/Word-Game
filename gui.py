...
# font_name = "/Assets/8-BIT WONDER.TTF"
...
def g_screen(md, df, word_list=[]):  # main display screen with text input
    ...
    while True:
        if pause_state == 1:
            ...
            if md == 0:
                text, lives, score, done = list_words(
                    text, lives, score, done, word_list
                )
            elif md == 1:
                text, lives, score, highest, done = list_highs(
                    text, lives, score, highest, done, word_list
                )
            if df == 0 or df == 2:
                prev_lives = lives_update(lives)
            prev_score = score_update(score)
            timer, pause_state = 999, 0

        if df == 1 or df == 2:
            timer, time = timer_update(timer, time)
        ...
        for event in p.event.get():
            if event.type == p.MOUSEBUTTONDOWN:
                m = p.mouse.get_pos()
                if collide(pause, m):
                    pause_panel(md, df, word_list)
                elif collide(shuff, m):
                    scramble = "".join(random.sample(scramble, len(scramble)))
            elif event.type == p.KEYDOWN:
        ...
        if sorted(done) == sorted(word_list):
            end_panel("WIN", md, df)
        elif lives == 0 or time == 0:
            end_panel("LOSE", md, df)
        ...
        p.draw.rect(
            d,
            c["black"],
            (sc[0] - t_b.get_rect().width / 2 + 20, 572, t_b.get_rect().width - 40, 42),
        )
        p.draw.rect(
            d,
            c["lgray"],
            (
                sc[0] - scram.get_rect().width / 2,
                475,
                scram.get_rect().width,
                scram.get_rect().height,
            ),
        )
        ...
        for event in p.event.get():
            if event.type == p.MOUSEBUTTONDOWN:
                ...
                if collide(resu, m):
                    ...
                    return g_screen(md, df, word_list)
                elif collide(menu, m):
                    menu_screen()
        p.display.update()
        z.clock.tick(z.frames)
...
def list_words(
    text, lives, score, done, word_list
):  # processes display for anagram mode
    i, y = 0, 100
    while i < len(word_list):
        if i == 0:
            if len(word_list) <= 4:
                x = sc[0] - 200
            elif 5 < len(word_list) < 11:
                x = 300
            else:
                x = 200
        if i != 0 and i % 5 == 0:
        ...
        if (text.lower() == word_list[i] or word_list[i] in done) and text != "":
            ...
            for j in range(len(word_list[i])):
                ...
                d.blit(
                    slot,
                    (
                        x + tile.get_rect().width / 2 - slot.get_rect().width / 2,
        ...
        x = def_x
        i, y = i + 1, y + 50
    if text != "" and text.lower() not in word_list:
        lives -= 1
...
def list_highs(
    text, lives, score, highest, done, word_list
):  # processes display for combine mode
    i, y = 0, 100
    if (
        text != ""
        and text.lower() in word_list
        and text.lower() not in highest
        and text.lower() not in done
    ):
        done.append(text.lower())
        if len(highest) < 6:
            ...
            while i < len(highest):
                if (
                    text != ""
                    and engine.scrabble_score(text.lower())
                    == engine.scrabble_score(highest[i])
                    and text.lower() not in highest
                ):
                    if text.lower() >= highest[i]:
                        highest.insert(i, text.lower())
                    else:
                        highest.insert(i + 1, text.lower())
                    ...
                    score, i = score + engine.scrabble_score(text.lower()), i + 1
                i += 1
    elif text != "" and text.lower() not in highest:
        lives -= 1
    highest, i = (
        sorted(highest[:6], key=lambda x: engine.scrabble_score(x), reverse=True),
        0,
    )
    p.draw.rect(d, c["lgray"], (sc[0] - 300, 100, 600, 275))
    while i < len(highest):
        word = f_f.render(
            highest[i] + "     " + str(engine.scrabble_score(highest[i])),
            True,
            c["white"],
        )
...
def button(x, y, off, on=0):  # button creation shortcut
    ...
    if on != 0:
        b_on = p.image.load(asset(on)).convert_alpha()
    w, h = b_off.get_rect().width, b_off.get_rect().height
    new_x, new_y = x - w / 2, y - h / 2
    if new_x + w > mouse[0] > new_x and new_y + h > mouse[1] > new_y and on != 0:
        d.blit(b_on, (new_x, new_y))
    else:
